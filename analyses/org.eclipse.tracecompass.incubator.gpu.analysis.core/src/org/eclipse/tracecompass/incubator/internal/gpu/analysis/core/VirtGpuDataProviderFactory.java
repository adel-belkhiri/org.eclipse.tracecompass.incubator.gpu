...
public class VirtGpuDataProviderFactory implements IDataProviderFactory {

    @Override
    public @Nullable ITmfTreeXYDataProvider <? extends ITmfTreeDataModel> createProvider(ITmfTrace trace) {
        Collection<ITmfTrace> traces = TmfTraceManager.getTraceSet(trace);
        if (traces.size() == 1) {
            return VirtGpuDataProvider.create(trace);
        }
        return TmfTreeXYCompositeDataProvider.create(traces, VirtGpuDataProvider.PROVIDER_TITLE, VirtGpuDataProvider.ID);
    }
